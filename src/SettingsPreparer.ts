class SettingsPreparer {
	...
	addHeader(): void {
		...
		const settingsPageJson: HtmlElement = {
			...
			children: [
				...
				},
				{
					element: "div",
					attributes: {
						class: "right-of-crumbs"
					},
					children: [
						{
							element: "div",
							attributes: {
								class: "TutorialToggleHolder"
							}
						}
					]
				}
		...
		const settingsPage: HTMLElement = this.utility.createHtmlFromJson(settingsPageJson);
		...
		target.insertBefore(settingsPage, target.firstChild);
	}
